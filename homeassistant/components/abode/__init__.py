...
AbodeConfigEntry: TypeAlias = ConfigEntry[AbodeSystem]
...
async def async_setup_entry(hass: HomeAssistant, entry: AbodeConfigEntry) -> bool:
    ...
    # Initialize global entity_ids set if not already present
    hass.data.setdefault(DOMAIN, {})
    hass.data[DOMAIN].setdefault("entity_ids", set())
...
async def async_unload_entry(hass: HomeAssistant, entry: AbodeConfigEntry) -> bool:
    ...
    if entry.runtime_data.logout_listener:
        entry.runtime_data.logout_listener()
